<template>
    ...
    <RadioGroup v-model="selectedDeliveryType">
        <RadioGroupLabel class=" text-gray-900"> روش ارسال را انتخاب کنید. </RadioGroupLabel>

        <div class="mt-4 grid grid-cols-2 gap-4">
            <RadioGroupOption
                :disabled="false"
                as="template" v-for="(deliveryType,index) in deliveryTypes" :key="index" :value="deliveryType.name" v-slot="{ checked, active }" >
                <div :class="[checked ? 'border-transparent' : 'border-gray-300', active ? 'ring-2 ring-indigo-500' : '', 'relative bg-white border rounded-lg shadow-sm p-4 flex cursor-pointer focus:outline-none']">
                    <div class="flex-1 flex">
                        <div class="flex flex-col">
                            <RadioGroupLabel as="span" class="block font-medium text-gray-900">
                                {{ deliveryType.name }}
                                <span v-if="deliveryType.city" class="text-xs">
                                                        ( تنها مخصوص شهر {{deliveryType.city}} )
                                                    </span>
                            </RadioGroupLabel>
                            <RadioGroupDescription as="span" class="mt-1 flex items-center text-xs text-gray-500">
                                {{ deliveryType.description }}
                            </RadioGroupDescription>
                            <RadioGroupDescription as="span" class="mt-6 text-sm  text-gray-900 ">
                                <div v-if="deliveryType.price===0" class="text-green-500">
                                    رایگان
                                </div>
                                <div v-else>
                                    {{ deliveryType.price.toLocaleString('fa') }} تومان
                                </div>
                            </RadioGroupDescription>
                        </div>
                    </div>
...
export default {
    ...
    setup(props,{emit}) {
        ...
        const deliveryTypes=Object.values(JSON.parse(props.deliveryTypes)).filter((dt)=> {
        ...
        return {
            provinces,deliveryTypes,selectedDeliveryType
