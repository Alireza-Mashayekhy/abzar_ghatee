<template>
    <Head>
        <title>خرید {{ name }} با بهترین قیمت | ابزارقطعه</title>
        <meta
            name="description"
            :content="
                'اطلاعات، قیمت و مشخصات انواع ' +
                name +
        ...
        <meta
            property="og:title"
            :content="'خرید' + name + 'با بهترین قیمت - ابزارقطعه'"
        />
        <meta
...
export default {
    ...
    setup({ productsJson, brandsJson, categoriesJson, name }) {
        const pageUrl = new URL(usePage().url, 'https://abzarghete.com');
        ...
        brands = brands.map((brand) => ({
            ...brand,
            checked:
                brandsParams.includes(brand.slug) || brandsParams.includes(),
        }));
